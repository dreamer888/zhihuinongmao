						...
						<div class="col-xs-12">
								<form action="seller/${msg }.do" name="userForm" id="userForm" method="post">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										<c:if test="${fx != 'head'}">
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">选择市场:</td>
											<td id="juese">
											<select class="chosen-select form-control" name="ROLE_ID" id="role_id" data-placeholder="请选择市场" style="vertical-align:top;" style="width:98%;" >
											<option value=""></option>
											<c:forEach items="${marketList}" var="role">
												<option value="${market.ID }" <c:if test="${market.ID == pd.ID }">selected</c:if>>${market.NAME }</option>
											</c:forEach>
											</select>
											</td>
										</tr>
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">选择分类:</td>
											<td id="juese">
											<select class="chosen-select form-control" name="ROLE_ID" id="role_id" data-placeholder="请选择分类" style="vertical-align:top;" style="width:98%;" >
											<option value=""></option>
											<c:forEach items="${marketList}" var="role">
												<option value="${market.ID }" <c:if test="${market.ID == pd.ID }">selected</c:if>>${market.NAME }</option>
											</c:forEach>
										...
										<c:if test="${fx == 'head'}">
											<input name="ROLE_ID" id="role_id" value="${pd.ID }" type="hidden" />
										</c:if>
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">商户账号:</td>
											<td><input type="text" name="USERNAME" id="loginname" value="${pd.NAME }" maxlength="32" placeholder="这里输入用户名" title="用户名" style="width:98%;"/></td>
										</tr>
										<tr>
